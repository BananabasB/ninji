...
(function(){
  ...
  // If computed-style matching fails, use these known hashed var names and their hex values.
  ...
  const applyTheme = () => {
    ...
    for(const [token, val] of Object.entries(theme)){
      ...
      // If still empty, try best-effort guess: map known hexes whose normalized hex startsWith the token color
      if(vars.length === 0){
        // attempt to pick hashed vars by hex guess -> token
        const guessed = [];
        for(const [hv, hex] of Object.entries(knownHashedColors)){
